...
const DEFAULT_GLOW_COLOR = '59, 130, 246'; // Blue to match TrustAI theme
...
const BentoCardWrapper: React.FC<BentoCardWrapperProps> = ({
    children,
    className = '',
    style,
    ...
    }, [animateParticles, clearAllParticles, enableTilt, enableMagnetism, enableBorderGlow, clickEffect, glowColor, enableParticles]);

    const glowStyles: React.CSSProperties = enableBorderGlow ? {
        '--glow-x': '50%',
        '--glow-y': '50%',
        '--glow-intensity': '0',
        '--glow-radius': '200px',
    ...
    return (
        <div
            ref={cardRef}
            className={`bento-effect-card ${enableBorderGlow ? 'bento-border-glow' : ''} ${className}`}
            style={{
                ...style,
                ...glowStyles,
                position: 'relative',
